...
class GoldTradingEnv(gym.Env):
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        # Normalisation des base features
        if NORMALIZE_OBSERVATIONS:
            if NORMALIZATION_METHOD == 'zscore':
                mean = np.nanmean(base_features)
                std = np.nanstd(base_features)
                if std > 0:
                    base_features = (base_features - mean) / std
                    base_features = np.clip(base_features, -5, 5)
            elif NORMALIZATION_METHOD == 'minmax':
                min_val = np.nanmin(base_features)
                max_val = np.nanmax(base_features)
                if max_val - min_val > 0:
                    base_features = 2 * (base_features - min_val) / (max_val - min_val) - 1
